...
public class LCS_DP_2
{
        ...
        for(int i=1;i<=s1.length();i++)
        {
            for(int j=1;j<=s2.length();j++)
            {
                if(s1.charAt(i-1)==s2.charAt(j-1))
                {
                    ...
                    else if(s[i-1][j-1].charAt(s[i-1][j-1].length()-1)<=s2.charAt(j-1))
                    {
    ...
    public static void main(String[] args)
    {
        try{
            ...
            System.out.println("\nTime taken to find Longest Common Increasing Subsequence is "+(endTime-startTime)+" nano seconds");
            System.out.println("\nLength of the Longest Common Increasing Subsequence is: "+b[strings[0].length()][strings[1].length()]);
